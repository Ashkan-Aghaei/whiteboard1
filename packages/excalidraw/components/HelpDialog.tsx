...
export const HelpDialog = ({ onClose }: { onClose?: () => void }) => {
  ...
  useEffect(() => {
    const global = JSON.parse(localStorage.getItem("DATA") || "{}");
    if (global?.region) {
      setRegion(+global?.region);
    }
  }, []);
  ...
  return (
    <>
      ...
      >
        {/* <Header /> */}
        {/* <a href={region ? "https://302.ai/" : "https://302ai.cn/"} style={{ color: '#8e47f0', fontSize: 24 }} >{t("officialWebsite.title")}</a> */}
        <Section title={t("helpDialog.shortcuts")}>
          ...
          >
            ...
            />
            <Shortcut label={t("toolBar.textToDiagram")} shortcuts={[KEYS.I]} />
            <Shortcut label={t("toolBar.magicframe")} shortcuts={[KEYS.C]} />
            <Shortcut label={t("toolBar.chat")} shortcuts={[KEYS.M]} />
